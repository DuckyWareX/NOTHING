...
function SectionTable:NewDropdown(drop)
    ...
    ValueText.ZIndex = 18
    ValueText.Font = Enum.Font.GothamBold
    ValueText.Text = tostring(drop.Default or "NONE")
    ValueText.TextColor3 = Color3.fromRGB(255, 255, 255)
    ValueText.TextScaled = true
    ValueText.TextSize = 14.000
    ValueText.TextTransparency = 0.500
    ...
    Button.Size = UDim2.new(0.08, 0, 0.45, 0)
    Button.Position = UDim2.new(0.97, 0, 0.35, 0)
    ...
    OptionTemplate.Size = UDim2.new(1, -8, 0, 26)
    OptionTemplate.AnchorPoint = Vector2.new(0.5,0)
    OptionTemplate.Position = UDim2.new(0.5, 0, 0, 4)
    OptionTemplate.BackgroundTransparency = 0.85
    OptionTemplate.AutoButtonColor = true
    OptionTemplate.Text = ""
    OptionTemplate.Font = Enum.Font.Gotham
    ...
    local function openList()
        ...
        for _, child in ipairs(List:GetChildren()) do
            if child:IsA("TextButton") and child ~= OptionTemplate then
                totalY = totalY + child.Size.Y.Offset + UIListLayout.Padding.Offset
            end
        end
        totalY = math.clamp(totalY, 0, 200)
        List.CanvasSize = UDim2.new(0, 0, 0, totalY)
        TweenService:Create(List, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, totalY)}):Play()
    end
    ...
    local function closeList()
        ...
        delay(0.16, function() List.Visible = false end)
    end
    ...
    FunctionDropdown.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            if opened then closeList() end
        end
    end)
    ...
    local function populate(data)
        ...
        for i, item in ipairs(data) do
            ...
            opt.MouseButton1Click:Connect(function()
                ValueText.Text = tostring(item)
                pcall(function() if drop.Callback then drop.Callback(item) end end)
                closeList()
            end)
        end
    end

    populate(drop.Data or {})

    function api:Update(newData, newDefault)
        populate(newData or {})
        if newDefault ~= nil then
            ValueText.Text = tostring(newDefault)
            pcall(function() if drop.Callback then drop.Callback(newDefault) end end)
        elseif (newData and #newData > 0) then
            ValueText.Text = tostring(newData[1])
            pcall(function() if drop.Callback then drop.Callback(newData[1]) end end)
        end
    end

    function api:Set(value)
        ValueText.Text = tostring(value)
        pcall(function() if drop.Callback then drop.Callback(value) end end)
    end
